...
public class LSMBTreeBulkloadEmptyStatisticsTest extends StatisticsTestDriver {

    private final OrderedIndexTestUtils orderedIndexTestUtils = new OrderedIndexTestUtils();

    public LSMBTreeBulkloadEmptyStatisticsTest() {
    ...
    protected void runTest(ISerializerDeserializer[] fieldSerdes, int numKeys, BTreeLeafFrameType leafType,
        ...
        //bulkloading empty component should still generate appropriate statistics message
        if (fieldSerdes[0] instanceof IntegerSerializerDeserializer) {
            orderedIndexTestUtils.bulkLoadIntTuples(ctx, 0, getRandom());
        } else if (fieldSerdes[0] instanceof UTF8StringSerializerDeserializer) {
            orderedIndexTestUtils.bulkLoadStringTuples(ctx, 0, getRandom());
        }
